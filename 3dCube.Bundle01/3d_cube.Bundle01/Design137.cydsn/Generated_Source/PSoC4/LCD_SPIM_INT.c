...
/* `#START LCD_SPIM_ISR_START_DEF` */

/* `#END` */


/*******************************************************************************
* Function Name: LCD_SPIM_TX_ISR
********************************************************************************
*
* Summary:
...
CY_ISR(LCD_SPIM_TX_ISR)
{
    ...
    /* User code required at start of ISR */
    /* `#START LCD_SPIM_TX_ISR_START` */

    /* `#END` */

    #if(LCD_SPIM_TX_SOFTWARE_BUF_ENABLED)
        /* Check if TX data buffer is not empty and there is space in TX FIFO */
        while(LCD_SPIM_txBufferRead != LCD_SPIM_txBufferWrite)
        {
            tmpStatus = LCD_SPIM_GET_STATUS_TX(LCD_SPIM_swStatusTx);
            LCD_SPIM_swStatusTx = tmpStatus;

            if(0u != (LCD_SPIM_swStatusTx & LCD_SPIM_STS_TX_FIFO_NOT_FULL))
            {
                if(0u == LCD_SPIM_txBufferFull)
                {
                ...
                CY_SET_REG16(LCD_SPIM_TXDATA_PTR,
                    LCD_SPIM_txBuffer[LCD_SPIM_txBufferRead]);
            }
            else
            {
                break;
...
CY_ISR(LCD_SPIM_RX_ISR)
{
    ...
    /* `#END` */
        ...
        LCD_SPIM_swStatusRx = tmpStatus;

        /* Check if RX data FIFO has some data to be moved into the RX Buffer */
        while(0u != (LCD_SPIM_swStatusRx & LCD_SPIM_STS_RX_FIFO_NOT_EMPTY))
        {
            rxData = CY_GET_REG16(LCD_SPIM_RXDATA_PTR);

            /* Set next pointer. */
            LCD_SPIM_rxBufferWrite++;
            if(LCD_SPIM_rxBufferWrite >= LCD_SPIM_RX_BUFFER_SIZE)
            {
                LCD_SPIM_rxBufferWrite = 0u;
            }

            if(LCD_SPIM_rxBufferWrite == LCD_SPIM_rxBufferRead)
            {
                ...
                LCD_SPIM_rxBufferFull = 1u;
            }

            /* Move data from the FIFO to the Buffer */
            LCD_SPIM_rxBuffer[LCD_SPIM_rxBufferWrite] = rxData;
